...
#let body = [
  An attack by #gls("dos") aims to render a service unavailable by overloading a device or network with messages #cite(<Wikipedia-DenialofserviceAttack2024>). In #gls("ot") environments, a capable computer can execute a #gls("dos") attack effectively. To reach this goal, a device sends a large number of messages to surcharge a device or a network. In @fig-attack-dos we can see that Mallory wants to overload Bob with messages. Bob is hence unable to answer to all messages and becomes unavailable.
  In #gls("it") world, it is more often a #gls("ddos") attack because usually servers are more powerful than a computer, and thus can handle more concurrent messages. The attacker distributes the messages on multiple devices to make the offense more difficult to block. 
  In #gls("ot") world, it is useless to perform a #gls("ddos") to make a device unavailable since a #gls("dos") with a capable computer is generally enough. Another perspective is that #gls("ot") world is typically in closed loop networks and thus not accessible from the outside world.
