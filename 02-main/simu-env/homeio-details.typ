...
#figure(
  image("homeio-door.png", width: 100%),
...
#figure(
  image("implementation.drawio.svg", width: 100%),
  caption: [Home I/O scene]
) <fig-simu-env-homeio-implementation>

As it's just a demonstrator, the controller send request for get status of both door sensors and the motion sensor. If one door is open or if a motion is detected, the controller send a request to the alarm to activate it. Otherwise, the controller send a request to the alarm to deactivate it.

The controller, the simulation and an attacker are connected to the same network via a switch. This thesis was made under a kali linux laptop but can be done with any computer with the right tools. 
