...
An other good idea would be to use the Electrical age world of Minecraft (@fig-simu-env-minecraft-eln-world) that was used in 2nd during previous Telecommunication course. The goal of this lab was to control a Factory and an energy system for produce the most as possible. It can be interesting for students to continue this lab to attack / secure the communication. On Minecraft, it is also possible to do security with Open Computer or even create an extension mod in Electrical age for Modbus over TLS (maybe more simple than using Open Computer on Lua).
