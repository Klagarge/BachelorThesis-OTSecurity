...
The first major challenge was the lack of tools for performing #gls("mitm") attacks below layer 7 (application layer). Most existing tools are designed for #gls("https") and are not applicable to this in this case, where the layer 7 is Modbus for this scenario. I spent a considerable amount of time trying to perform a #gls("mitm") attack on #gls("tls") on the fly. After much effort, I eventually realised that setting up a server-client bridge and providing a fake certificate was a better approach. Something that now seems obvious in hindsight. Ultimately, this is the solution I implemented.

The second significant difficulty was related to the differences in modulation between #gls("wmbus") and the #gls("flipper") device. It took me a long time to recognise that the issue was not due to poor transceiver configuration on my part. After extensive attempts to measure and analyse the radio frequency signal, I eventually realised that my misunderstanding of the #gls("flipper") documentation was the root cause. I then spent additional time attempting to configure the TI chip in the #gls("flipper"). Moving on to Plan B after investing so much time and effort in the initial approach was difficult.
...
For the replay scenario, it could be interesting to connect it to the Wago CC100 to trigger the garage door in the Home I/O simulation, making the scenario more realistic and engaging for students. However, an interface between the 433 MHz transceiver and the Wago CC100 would need to be developed, along with a remote control for student to use. This scenario remains more of a proof of concept, as it currently relies on serial communication with a *Go* software.
