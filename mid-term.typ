...
//----------------
= Introduction
At #school.shortname, students specializing in #school.specialisation take a course on #gls("ot") Security. This course is somewhat outdated and will be reworked for the year to come. The aim of my Bachelor's thesis is to come up with several attack scenarios which can be used as the basis for a laboratory module for students. These scenarios could also serve in industrial training partnerships with #school.shortname. I will implement between three and four attack scenarios, including at least one wireless attack and one Modbus attack.

I chose this thesis subject due to my keen interest in Security and PEN-testing. Additionally, I already have some basic ethical hacking background and would like to specialize in security of embedded systems during the rest of my studies. This thesis presents a valuable opportunity to delve into this field, and I have already enrolled for a master's degree to continue on this path.

This midterm report presents the planning of the thesis, an analysis of various attacks, communication media and simulation environments, and a conclusion outlining the next steps.
...
+ *Replay attack on a wireless device.* This attack will probably be carried out on a #gls("wmbus", long: true) electricity meter. If this proves too challenging within the available time, an alternative would be to target the garage door remote control using simple #gls("ook") modulation on 433 MHz. One potential countermeasure to protect against this attack, to be confirmed, would be to implement a rolling code.
