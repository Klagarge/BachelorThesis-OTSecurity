...
+ *Replay attack on a wireless device.* This attack will be probably carried out on a #gls("wmbus", long: true) electricity meter. If this proves too challenging within the available time, an alternative would be to target the garage door remote control using simple #gls("ook") modulation on 433 MHz. One potention contermeasure to protect against this attack, have to be confirmed later, would be implementing a rolling code.

+ *Denial of service attack on an external sensor.* This attack will inlvolve flooding the #gls("plc") with valid messages at a frequency too high for the #gls("plc") to handle, effectively overloading it. To mitigate this, a better programming design will have to be developed.

+ *#gls("mitm", long: true) attacks.* Two phases of #gls("mitm") attack will be conductedcommunication. The first will target an unencrapted Modbus #gls("tcp") communication. Protection against this will involve implementing a symmetrical key exchanged using #gls("d-h"). The second phase will exploit the key exchange process, demonstrating that if an attacker is present from the start, it is impossible to guarantee the authenticity of the communication partner. utilizing certificates is the only way to ensure secure communication. So Modbus over #gls("tls") will be used instead of #gls("tcp") to protect against this attack.
